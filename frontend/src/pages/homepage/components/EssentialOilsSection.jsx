...
const EssentialOilsSection = () => {
  ...
  useEffect(() => {
    const fetchProductIds = async () => {
      try {
        const updatedOils = await Promise.all(essentialOils.map(async (oil) => {
          try {
        ...
        setEssentialOils(updatedOils);
      } catch (error) {
        console.error("Error updating Essential Oils with backend IDs:", error);
      }
    };
